...
public static class TableLevelAssetBuilder
{
    ...
    static void EnsureAssets()
    {
        ...
        if(!File.Exists(Root+"Resources/TableLevels.asset")) Build();
    }
    [MenuItem("Tools/Table Levels/Create missing level assets")]
    public static void Build()
    {
        foreach(var folder in new[]{"Resources","Town/Lighting","Dungeon1/Lighting","Dungeon1/Materials","Dungeon1/Enemies","Dungeon1/Loot","Dungeon1/Prefabs"}) Directory.CreateDirectory(Root+folder);
        ...
        var loot=Asset<DungeonLootTable>(Dungeon+"Loot/Crypt supplies.asset");
    ...
    {
        string path=Dungeon+"Loot/"+name+".asset";
        var item=AssetDatabase.LoadAssetAtPath<ItemData>(path);if(item!=null)return item;
        item=UnityEngine.Object.Instantiate(AssetDatabase.LoadAssetAtPath<ItemData>("Assets/Game/Items/Content/"+original+".asset"));
    ...
    static GameObject Enemy(string name,float hp,float damage,float speed,float cooldown,string model,ItemData carried,DungeonLootTable loot)
    {
        ...
        if(animator.isHuman)
        {
            ...
            if(hand!=null && carried.worldPrefab!=null)
            {
                var weapon=UnityEngine.Object.Instantiate(carried.worldPrefab,hand);weapon.transform.localPosition=carried.heldPosition;weapon.transform.localRotation=Quaternion.Euler(carried.heldRotation);
                foreach(var col in weapon.GetComponentsInChildren<Collider>())UnityEngine.Object.DestroyImmediate(col);
            }
        }
