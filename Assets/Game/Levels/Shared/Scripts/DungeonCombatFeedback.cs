...
public class DungeonCombatFeedback : MonoBehaviour
{
    ...
    public void Initialize(DungeonGenerator generator, RectTransform parent, TMP_FontAsset pixelFont)
    {
        ...
        message=Label(canvas,"Dungeon message",28);SetRect(message.rectTransform,new Vector2(.5f,0),new Vector2(0,115),new Vector2(850,36));
        impact=Label(canvas,"Combat result",25);SetRect(impact.rectTransform,new Vector2(.5f,.5f),new Vector2(0,-46),new Vector2(300,34));
        for(int i=0;i<24;i++)
        {
            var label=Label(canvas,"Damage number",29);label.gameObject.SetActive(false);
            label.rectTransform.sizeDelta=new Vector2(160,44);popups.Add(new Popup{label=label});
        }
    }
    void Register(Character character)
    {
        ...
        var frame=Image(canvas,"Enemy health frame",new Color(.62f,.49f,.27f));
        var root=frame.rectTransform;root.sizeDelta=new Vector2(180,17);
        var bg=Image(root,"Health background",new Color(.035f,.045f,.04f));Stretch(bg.rectTransform,2);
        var loss=Image(bg.rectTransform,"Recent damage",new Color(1,.77f,.36f));Stretch(loss.rectTransform,0);
        var fill=Image(bg.rectTransform,"Health remaining",new Color(.7f,.22f,.09f));Stretch(fill.rectTransform,0);
        var name=Label(root,"Enemy name",22);name.text=character.DisplayName;
        SetRect(name.rectTransform,new Vector2(.5f,1),new Vector2(0,19),new Vector2(260,28));
        var view=new EnemyView{character=character,root=root,fill=fill,loss=loss};
    ...
    TMP_Text Label(Transform parent,string name,float size)
    {
        var go=new GameObject(name,typeof(RectTransform),typeof(TextMeshProUGUI));go.transform.SetParent(parent,false);
        var text=go.GetComponent<TextMeshProUGUI>();if(font!=null)text.font=font;text.fontSize=size;text.alignment=TextAlignmentOptions.Center;text.raycastTarget=false;text.color=new Color(.96f,.89f,.72f);
        var shadow=go.AddComponent<Shadow>();shadow.effectColor=Color.black;shadow.effectDistance=new Vector2(2,-2);return text;
    }
    static Image Image(Transform parent,string name,Color color)
    {
        var go=new GameObject(name,typeof(RectTransform),typeof(Image));go.transform.SetParent(parent,false);var image=go.GetComponent<Image>();image.color=color;image.raycastTarget=false;return image;
    }
    ...
    static void SetRect(RectTransform r,Vector2 anchor,Vector2 position,Vector2 size){r.anchorMin=r.anchorMax=anchor;r.anchoredPosition=position;r.sizeDelta=size;}
    void OnDisable()
    {
