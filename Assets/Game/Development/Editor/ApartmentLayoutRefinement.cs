...
public static class ApartmentLayoutRefinement
{
    ...
    public static void Build()
    {
        ...
        if(scene.path!="Assets/Game/Scenes/Room.unity" || EditorApplication.isPlaying) throw new InvalidOperationException("Open Room in edit mode.");
        ...
        wood=(root.Find("Main Bedroom/Geometry/Floor") ?? root.Find("Existing Bedroom/Floor")).GetComponent<Renderer>().sharedMaterial;
    ...
    static Material Mat(string name,Color c) {
        string path="Assets/Game/World/Materials/Apartment/"+name+".mat";var m=AssetDatabase.LoadAssetAtPath<Material>(path);
    ...
    public static void RenderViews() {
        ...
        bool expanded=(root.Find("Second Bedroom/Geometry/Floor") ?? root.Find("Second Bedroom/Floor")).GetComponent<Renderer>().bounds.max.z>540;
...
{
    const string Folder="Assets/Game/World/Prefabs";
    ...
    {
        if(EditorApplication.isPlaying || UnityEngine.SceneManagement.SceneManager.GetActiveScene().path!="Assets/Game/Scenes/Room.unity")throw new InvalidOperationException("Open Room in edit mode.");
        var apartment=GameObject.Find("Apartment").transform;
        ...
        var floor=(apartment.Find("Entrance and Hall/Geometry/Floor entrance recess") ?? apartment.Find("Entrance and Hall/Floor entrance recess")).GetComponent<Renderer>().sharedMaterial;
        var wall=AssetDatabase.LoadAssetAtPath<Material>("Assets/Game/World/Materials/Apartment/Hall warm plaster.mat");
    ...
    static AudioData AudioAsset(string name,params string[] clips) {
        string path="Assets/Game/Audio/Data/"+name+".asset";var data=AssetDatabase.LoadAssetAtPath<AudioData>(path);
        if(!data){data=ScriptableObject.CreateInstance<AudioData>();data.clips=clips.Select(c=>AssetDatabase.LoadAssetAtPath<AudioClip>("Assets/Game/Audio/Library/_Sound Effects Library/Doors/"+c)).ToArray();data.volume=.8f;data.pitchVariance=.025f;data.minDistance=20;data.maxDistance=240;AssetDatabase.CreateAsset(data,path);}return data;
    }
    ...
    public static void AddFrame(GameObject door) {
        ...
        var mat=AssetDatabase.LoadAssetAtPath<Material>("Assets/Game/World/Materials/Apartment/Architectural trim.mat");
