namespace MFPC
{
    ...
    {
        #region Variables
        public static PlayerController_Legacy instance;
        ...
        [Tooltip("Key used to lean to the left.")]
        [SerializeField] public KeyCode leftLeanKey = KeyCode.Q;
        [Tooltip("Key used to lean to the right.")]
        [SerializeField] public KeyCode rightLeanKey = KeyCode.E;
        [Tooltip("Key used to lean to sprint.")]
        [SerializeField] public KeyCode sprintKey = KeyCode.LeftShift;
        [Tooltip("Key used to lean to crouch.")]
        [SerializeField] public KeyCode crouchKey = KeyCode.LeftControl;
        [Tooltip("Key used to lean to jump.")]
        [SerializeField] private KeyCode jumpKey = KeyCode.Space;
        [Tooltip("Key used to lean to aim.")]
        ...
        public bool IsLeftLeaning => (gameData == null || gameData.enableLean) && Input.GetKey(leftLeanKey);
        public bool IsRightLeaning => (gameData == null || gameData.enableLean) && Input.GetKey(rightLeanKey);
        ...
        void HandleMovementStates()
        {
            ...
            bool crouchInputLocksCrouch = gameData.holdToCrouch && crouchHeld;
        ...
        void PlayerMovement()
        {
            ...
            bool canApplySprint =
                ...
                moveDir.z > 0 &&
                !(leanEnabled && Input.GetKey(leftLeanKey)) &&
                !(leanEnabled && Input.GetKey(rightLeanKey));
            ...
            // -------- Jump --------
            if (Input.GetKeyDown(jumpKey) && Controller.isGrounded && TryConsumeJumpStamina())
            {
        ...
        void CameraLeanAndRotation()
        {
            ...
            // ROTATION
            float sensitivity = (gameData.mouseSensitivity / 5);
            if (gameData.invertLook == false)
            {
                LookInput = new Vector2(Input.GetAxisRaw("Mouse X"), Input.GetAxisRaw("Mouse Y")) * sensitivity;
            }
            else
            {
                LookInput = new Vector2(Input.GetAxisRaw("Mouse X"), -Input.GetAxisRaw("Mouse Y")) * sensitivity;
            }
            float YRot = verticalNeck.rotation.eulerAngles.y + LookInput.x;

            verticalRotStore += LookInput.y;
            verticalRotStore = Mathf.Clamp(verticalRotStore, minClamp, maxClamp);

            // LEANING
            float targetLean = 0f;
            if (leanEnabled && Input.GetKey(leftLeanKey))
                targetLean = leanAmount;
            else if (leanEnabled && Input.GetKey(rightLeanKey))
                targetLean = -leanAmount;

            // Smoothly interpolate toward target lean (FPS independent)
            float smoothSpeed = (targetLean == 0) ? lateralReturnTime : lateralSmoothTime;
            float t = 1f - Mathf.Exp(-smoothSpeed * Time.deltaTime);
            currentLeanAngle = Mathf.Lerp(currentLeanAngle, targetLean, t);
        ...
        public void SetInState(bool value)
        {
            InWater = value;
        }
        #endregion
        ...
        {
            float volume = (IsCrouching ? crouchVolumeReduction : 1); // volume of sfx when the player is crouched is reduced
